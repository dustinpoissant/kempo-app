...
export default {
  ...
  'AppSetting loads value from db on connect': async ({ pass, fail }) => {
    await window.api.jsonDB("settings").set("testSetting", "hello");
    const el = document.createElement("app-setting-string");
  ...
  'AppSetting saves value to db when value changes': async ({ pass, fail }) => {
    await window.api.jsonDB("settings").set("saveSetting", "old");
    const el = document.createElement("app-setting-string");
    ...
    await el.updateComplete;
    await new Promise(r => setTimeout(r, 100));
    const stored = await window.api.jsonDB("settings").get("saveSetting");
    if(stored === "new") pass();
